...
const useFluidCursor = (canvas: HTMLCanvasElement, theme: 'dark' | 'light') => {
  ...
  function getWebGLContext(canvas: HTMLCanvasElement) {
    ...
    // Always clear to transparent so CSS background shows through
  ...
  function update() {
    // Efficiency: Pause when tab is hidden
    if (document.hidden) {
  ...
  // PERF FIX: Only resize on window event
  const handleResize = () => {
    if (resizeCanvas()) {
       initFramebuffers();
    }
  ...
  addListener(window, 'mousedown', mousedownHandler);
  ...
  addListener(window, 'touchstart', touchStartHandler);
  ...
  addListener(window, 'touchmove', touchMoveHandler);
  ...
  addListener(window, 'touchend', touchEndHandler);
